...
#pragma omp section
            {
                // один поток захватил замок A, в этот момент другой
                // захватил B, в итоге первый поток ждет освобождения замка B
                // пока второй поток ждет освобождения замка A
                // замок в секции не нужен, тк секцию и так выполняет 1 поток
                //omp_set_lock(&locka);
